...
ggplot(full_terminal_depletion %>% filter(names == max(full_years)), aes(x = model.x, group = model.x)) +
  ...
  geom_hline(yintercept = 0, col = "red", linewidth = 1.1, linetype = "dashed") +
  theme_bw() +
  labs(x = "EM", y = "Relative error in depletion in terminal year") +
  facet_grid(rebuild~init) +
  theme(axis.text.x = element_text(angle = 90))
...
## calculate RE
full_survey_AF = full_survey_AF %>% group_by(Age, sim_iter, init, rebuild, year, model.x) %>% mutate(RE = (fit.x - fit.y)/ fit.y * 100)
...
ggplot(full_survey_AF %>% filter(model.x == "EM1_00", year == 1961), aes(x = Age, group = Age)) +
  geom_boxplot() +
  geom_hline(yintercept = 0, linetype = "dashed", col = "red") +
  theme_bw() +
  ggtitle("EM1_00") +
