...
TMB_data$fishery_AF_obs = array(10, dim = c(TMB_data$n_ages, sum(TMB_data$fishery_year_indicator), TMB_data$n_fisheries))
...
TMB_data$ycs_estimated[(length(TMB_data$ycs_estimated) - 5):length(TMB_data$ycs_estimated)] = 0
## simulate recruitment
OM_pars$ln_ycs_est = rnorm(sum(TMB_data$ycs_estimated),  -0.5*this_bio$sigma_r^2, this_bio$sigma_r)
...
for(start_year_ndx in 1:length(start_year)) {
  ...
  for(sim_iter in 1:n_sims) {
    ...
    ## save SSB
    this_ssb = data.frame(years = full_years, OM_SSB = OM_sim$ssb, EM_SSB = EM_rep$ssb, sim_iter = sim_iter, start_year = start_year[start_year_ndx])
    SSB_df = rbind(SSB_df, this_ssb)
    ## this depletion
    this_depletion = data.frame(years = full_years, OM_depletion = OM_sim$depletion, EM_depletion = EM_rep$depletion, sim_iter = sim_iter, start_year = start_year[start_year_ndx])
    depletion_df = rbind(depletion_df, this_depletion)
    ## save reccruitment
    this_recruit = data.frame(years = years, OM_ycs = OM_sim$ycs,EM_ycs = EM_rep$ycs, sim_iter = sim_iter, start_year = start_year[start_year_ndx])
    recruit_df = rbind(recruit_df, this_recruit)
    ## selectivities
    ...
    ## survey abundance
    tmp_srv_abundance = data.frame(years = sim_survey_year_obs, SE = EM_rep$survey_sd, fitted = EM_rep$survey_index_fitted, observed = EM_data$survey_obs, sim_iter = sim_iter, start_year = start_year[start_year_ndx])
