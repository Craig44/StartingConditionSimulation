...
include_SE = T
N_eff = 150
OM_label = "Fast_sensitivity_biennial_OM3"
...
TMB_data$max_age_ndx_for_AFs = max(this_bio$ages)
...
for(init_ndx in 1:length(inital_levels)) {
  ...
  for(rebuild_ndx in 1:length(rebuild_levels)) {
    ...
    for(sim_iter in 1:n_sims) {
      ...
      EM1c_obj <- MakeADFun(EM1c_data, EM_pars, map = na_EM1_pars, DLL= "AgeStructuredModel", checkParameterOrder = T, silent = T)
      EM1d_obj <- MakeADFun(EM1d_data, EM_pars, map = na_EM1_pars, DLL= "AgeStructuredModel", checkParameterOrder = T, silent = T)
