...
Type objective_function<Type>::operator() () {
  ...
  int n_fisheries = 1;
  DATA_IVECTOR(fixed_catch_indicator);  // Switches between F-based catch and F-derived from given catch
  ...
  array<Type> catches(n_years, n_fisheries); 
  ...
  for(year_ndx = 1; year_ndx <= n_years; year_ndx++) {
    ...
    // -- adjust
    if(fixed_catch_indicator(year_ndx - 1) == 1) {
      // This years F is based on the constant Catch
      // Check if we have set the constant catch
      if(constant_catch <= 0) 
        constant_catch = pred_catches(year_ndx - 2, 0);
      pred_catches(year_ndx - 1, 0) = constant_catch;
      // calculate vulnerable and initial calculations
      for(fishery_ndx = 0; fishery_ndx < n_fisheries; ++fishery_ndx) {
        total_catch_this_year = constant_catch;
        ...
        init_popes_rate(fishery_ndx) = constant_catch / (vulnerable_bio(fishery_ndx) + 0.1 * constant_catch); //  Pope's rate  robust A.1.22 of SS appendix
      ...
      for(int f_iter = 0; f_iter < F_iterations; ++f_iter) {
        ...
        for(fishery_ndx = 0; fishery_ndx < n_fisheries; ++fishery_ndx) {
          ...
          exploitation_rate(fishery_ndx) = constant_catch / (vulnerable_bio(fishery_ndx) + 0.0001); //  Pope's rate  robust A.1.22 of SS appendix
    ...
    // Else Use input F's to derive Catch
